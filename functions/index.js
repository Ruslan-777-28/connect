...
exports.dailyWebhook = onRequest(
  ...
  async (req, res) => {
    ...
    if (!signature || !timestamp) {
      logger.warn("Webhook received without signature or timestamp");
    ...
    try {
      ...
      if (!crypto.timingSafeEqual(Buffer.from(signature), Buffer.from(expectedSignature))) {
        ...
        logger.error("Error during webhook signature verification", err);
    ...
    try {
        const { id: eventId, type, payload } = req.body;
        if (!type || !payload?.payload?.room || !eventId) {
            logger.log("Webhook event is missing required fields", { body: req.body });
            return;
        }

        const roomName = payload.payload.room;
        const userId = payload.payload.user_id;

        const roomMapSnap = await admin.firestore().collection("dailyRooms").doc(roomName).get();
        if (!roomMapSnap.exists) {
            logger.warn(`dailyRooms mapping not found for room: ${roomName}`);
            return;
        }
        const { callId } = roomMapSnap.data();
        const callRef = admin.firestore().doc(`calls/${callId}`);
        ...
        await admin.firestore().runTransaction(async (tx) => {
            ...
            if (eventSnap.exists) {
                logger.log(`Webhook event ${eventId} already processed for call ${callId}`);
                return;
            }

            const callSnap = await tx.get(callRef);
            if (!callSnap.exists) {
                logger.warn(`Call document ${callId} not found`);
                return;
            }
            const call = callSnap.data();

            if (call.status !== "accepted") {
                logger.log(`Ignoring webhook for call ${callId} with status ${call.status}`);
                return;
            }
            ...
            if (type === "participant-joined") {
                logger.log(`Processing participant-joined for call ${callId}`, { userId });
                ...
                if (!currentParticipants[userId]) {
                    updatePayload = {
                        ...
                        [`participants.${userId}`]: { joinedAt: admin.firestore.FieldValue.serverTimestamp() },
                        lastPresenceAt: admin.firestore.FieldValue.serverTimestamp(),
                    };
                }
            } else if (type === "participant-left") {
                logger.log(`Processing participant-left for call ${callId}`, { userId });
                const currentParticipants = call.participants || {};
                if (currentParticipants[userId]) {
                     const newActiveCount = Math.max(0, (call.activeCount || 1) - 1);
                     updatePayload = {
                     ...
                     if (newActiveCount === 0) {
                         logger.log(`Last participant left call ${callId}. Ending call.`);
                         updatePayload = {
                            ...updatePayload,
                            status: "ended",
                            endReason: "left",
            ...
            if (Object.keys(updatePayload).length > 0) {
                tx.update(callRef, updatePayload);
            }
